...
public class ScrewsInv {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ScrewsInv window = new ScrewsInv();
	...
	private JLabel lblScrewsInventory;
	...
	private void initialize() {
		...
		comboboxTools.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try{
					String query = " select * from DiffScrews where ID=?";
					PreparedStatement pst = connection.prepareStatement(query);
		...
		btnSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try{
					String query = "insert into DiffScrews (Type,Quantity,Location) values (?,?,?)";
					PreparedStatement pst = connection.prepareStatement(query);
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try{	
					String query = "Update DiffScrews set ID = '"+textFieldInvID.getText()+"', Type = '"+textFieldType.getText() + "', Quantity = '"+textFieldAmount.getText()+"', Location = '"+textFieldPrice.getText()+"' where ID = '"+textFieldInvID.getText() + "' ";
					PreparedStatement pst = connection.prepareStatement(query);
					pst.execute();
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try{
					String query = "delete from DiffScrews where ID = '"+comboboxTools.getToolTipText()+"' ";
					PreparedStatement pst = connection.prepareStatement(query);
	...
	tableInv.addMouseListener(new MouseAdapter() {
		...
		public void mouseClicked(MouseEvent e) {
			try{
				...
				String query = " select * from DiffScrews where ID='"+Name_+"' ";
				PreparedStatement pst = connection.prepareStatement(query);
	...
	lblScrewsInventory = new JLabel("Screws Inventory:");
	lblScrewsInventory.setFont(new Font("Tahoma", Font.PLAIN, 20));
	lblScrewsInventory.setBounds(12, 13, 228, 47);
	frame.getContentPane().add(lblScrewsInventory);
	
	textFieldInvID = new JTextField();
	textFieldInvID.setColumns(10);
	textFieldInvID.setBounds(57, 107, 64, 22);
